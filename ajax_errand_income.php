...
if(isset($_POST["input"])){
	...
	$input 		= 	$_POST["input"];
	$errand_id	=	$_SESSION[WEB_NAME]['login_id'];
	
	$get_delfee 	= $DB_Helper->get_row("SELECT * FROM tbl_errand WHERE id='$errand_id'");
	$del_fee	= $get_delfee->del_fee;
	
	if($select_val == "VN"){
		
		$sql_fields = "";
		$sql_fields_1 = " AND B.firstname LIKE '{$input}%' OR B.lastname LIKE '{$input}%' ";
	...
	$tablename	= 	TABLE_PREFIX.'remittance';
		...
		$query = "SELECT A.* , CONCAT(B.firstname ,' ', B.mi,'. ', ' ', B.lastname) AS v_name, 
					CONCAT(C.firstname ,' ', C.mi,'. ', ' ', C.lastname) AS e_name,
						...
						(SELECT COUNT(id) 
						FROM tbl_remittance WHERE errand_id = '$errand_id' $sql_fields $sql_fields_1) AS total_data,
						
							(SELECT SUM(total_amt) 
							FROM tbl_remittance AS D
							INNER JOIN tbl_vendor AS E ON (D.`vendor_id` = E.`id`) WHERE errand_id = '$errand_id' $sql_fields $sql_fields_2) AS tots_price,
							
								(SELECT COUNT(F.id) 
								FROM tbl_remittance AS F
								INNER JOIN tbl_vendor AS G ON (F.`vendor_id` = G.`id`) WHERE errand_id = '$errand_id' $sql_fields $sql_fields_3) AS tots_record,
								
									(SELECT COUNT(id) 
									FROM tbl_remittance WHERE errand_id = '$errand_id') AS all_record
									
					FROM tbl_remittance AS A
					INNER JOIN tbl_vendor AS B ON (A.`vendor_id` = B.`id`)
					INNER JOIN tbl_errand AS C ON (A.`errand_id` = C.`id`)
					WHERE A.`errand_id` = '$errand_id' $sql_fields $sql_fields_1
					ORDER BY A.`id` DESC";
		//echo $query;
